...
class _productgrid1State extends State<productgrid1> {
  ...
  Map<String, Map<String, dynamic>> productCache = {}; // Cache for product data
  final GoogleTranslator _translator = GoogleTranslator(); // Initialize the translator
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final productData = {
          ...
          'productName': product['product_name'] ?? 'No product name available',
          'quantity': product['quantity'] ?? 'No quantity available',
          'packaging':
              product['packaging'] ?? 'No packaging information available',
          ...
          'manufacturingPlaces': (product['manufacturing_places'] is List)
              ...
              : product['manufacturing_places'] ??
                  'No manufacturing place available',
        };

        
        final translatedName = await _translator.translate(productData['productName'], to: 'en');
        final translatedCategories = await _translator.translate(productData['categories'], to: 'en');
        final translatedBrands = await _translator.translate(productData['brands'], to: 'en');
        
        productData['productName'] = translatedName.text;
        productData['categories'] = translatedCategories.text;
        productData['brands'] = translatedBrands.text;
  ...
  Future<void> fetchBarcodeList() async {
    ...
    try {
      final snapshot =
          await firestore.collection('barcodes').doc('barcode_data').get();
              ...
              itemBuilder: (context, index) {
                return FutureBuilder<Map<String, dynamic>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final price = product['price'] ?? 'No price available';
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    product['quantity'] ??
                                        'No quantity available', // Added quantity here
                                    style: const TextStyle(
                                      fontSize: 12.0,
                                      color: Colors
...
void main() {
  runApp(const MaterialApp(home: productgrid1()));
}
