...
import styles from "./ConsoleBundleDescription.module.css";
import { useLocation, useNavigate } from "react-router-dom";
import { CartContext } from "../../../../pages/addtocart/CartContext";
import { FaStar } from "react-icons/fa";

export default function ConsoleBundleDescription() {
    const location = useLocation();
    const navigate = useNavigate();
    const { addToCart } = useContext(CartContext);

    const item = location.state?.bundle;

    if (!item) {
        return (
            <div className={styles["not-found"]}>
                <h2>Item Not Found</h2>
                <button onClick={() => navigate("/ConsoleBundle")}>Go Back</button>
            </div>
        );
    }
